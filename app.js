(() => {
  ...
  function renderMarketChart(ticker){
    ...
    try{
      const theme=chartStyles();box.style.background=theme.bg;window.klinecharts.registerLocale('cs-CZ',{time:'Čas',open:'Otevření',high:'Maximum',low:'Minimum',close:'Zavření',volume:'Objem',change:'Změna',turnover:'Obrat'});const chart=window.klinecharts.init(box);if(!chart)throw new Error('Graf nelze inicializovat');state.chart=chart;chart.setLocale('cs-CZ');chart.setTimezone('Europe/Prague');chart.setStyles(theme.styles);const last=shown.at(-1)?.close||0;chart.setPriceVolumePrecision(last<1?4:2,0);const indicator=state.chartIndicator;if(indicator){if(['MA','EMA','BOLL'].includes(indicator))chart.createIndicator(indicator,false,{id:'candle_pane'});else chart.createIndicator(indicator,false,{height:125,minHeight:80});}chart.applyNewData(shown);chart.scrollToRealTime();$('#chartMeta').textContent=`${shown.length} svíček · interval ${interval==='D'?'1D':interval+'m'} · čas Praha · data přibližně 15 min zpožděná`;
