(() => {
  ...
  async function openDetail(ticker){
    ...
    try{const [bars,intraday,hourly,sec,fund,spy]=await Promise.all([data(`bars/${ticker}.json`),data(`intraday/${ticker}.json`).catch(()=>[]),data(`hourly/${ticker}.json`).catch(()=>[]),data(`t/${ticker}.json`).catch(()=>null),data(`fundamentals/${ticker}.json`).catch(()=>null),data('bars/SPY.json').catch(()=>[])]);state.detailBars=bars||[];state.detailIntraday=intraday||[];state.detailHourly=hourly||[];state.detailFundamentals=fund;renderMarketChart(ticker);renderDetailInsiders(sec?.tx||[]);renderEvents(s,sec?.tx||[],bars||[],fund?.events||[]);renderFundamentals(fund,s);renderRiskPanel(bars||[],spy||[]);}catch{state.detailBars=[];state.detailIntraday=[];state.detailHourly=[];state.detailFundamentals=null;renderMarketChart(ticker);renderDetailInsiders([]);renderEvents(s,[],[]);renderFundamentals(null,s);renderRiskPanel([],[]);}
  }
  ...
  function timestampChartData(rows){return (rows||[]).filter(r=>Array.isArray(r)&&r.length>=6).map(r=>({timestamp:new Date(r[0]).getTime(),open:+r[1],high:+r[2],low:+r[3],close:+r[4],volume:+r[5]||0})).filter(r=>Number.isFinite(r.timestamp));}
  ...
  function renderMarketChart(ticker){
    ...
    const interval=state.chartInterval,minutes=interval==='D'?null:Number(interval),bars=interval==='D'?dailyChartData(state.detailBars):interval==='60'&&state.detailHourly.length?timestampChartData(state.detailHourly):aggregateIntraday(state.detailIntraday,minutes);const shown=interval==='D'?bars.slice(-800):bars;
